...
import {addAccount, getAccounts} from "@/actions/financeAction"
      ...
      >
        <Form {...formItemLayout} >
          ...
          <Form.Item label="账户类型" >
            <Radio.Group value={type} onChange={({target})=>this.setState({type: target.value})}>
          ...
          <Form.Item label="是否关联" hasFeedback>
            <Radio.Group value={linkType} onChange={({target})=>this.setState({linkType: target.value})}>
              <Radio.Button value="0">不关联</Radio.Button>
              <Radio.Button value="1">关联项目</Radio.Button>
            </Radio.Group>
          </Form.Item>
          
          {linkType=="1"?<Form.Item label="关联项目" hasFeedback>
...
function mapDispatchProps(dispatch){
  return {
    actions: bindActionCreators({addAccount, getAccounts}, dispatch)
  }
}
