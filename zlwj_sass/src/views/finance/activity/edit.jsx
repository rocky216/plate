...
import {getCostActive, editCostActive} from "@/actions/financeAction"
...
class EditActivity extends React.Component {
  ...
  handlenSubmit(){
    this.props.form.validateFieldsAndScroll((err, values)=>{
      ...
      if(!err){
        this.props.actions.editCostActive({
          ...values,
        ...
        <Form {...formItemLayout} >
          <Form.Item label="活动名称" >
            {getFieldDecorator('activeName', {
              initialValue: detail.activeName,
              rules: [
                {
                  required: true,
                  message: '填写活动名称!',
          ...
          <Form.Item label="奖励类型" >
            {getFieldDecorator('activeType', {
              initialValue: detail.activeType,
              rules: [
                {
                  required: true,
                  message: '奖励类型!',
          ...
          <Form.Item label="条件类型" >
            {getFieldDecorator('conditionsType', {
              initialValue: detail.conditionsType,
              rules: [
                {
                  required: true,
                  message: '条件类型!',
...
function mapDispatchProps(dispatch){
  return {
    actions: bindActionCreators({getCostActive, editCostActive}, dispatch)
  }
}
...
export default connect(mapStateProps, mapDispatchProps)(Form.create()(EditActivity))