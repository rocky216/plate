...
import {initBasePropertyOrder,initLoadOrderMoney,EditFixPlan, propertyPlanPage} from "@/actions/otherAction"
...
class EditTrim extends React.Component {
  constructor(props){
    super(props)
    this.state = {
      owners: "",
  ...
  componentDidMount(){
    ...
    this.props.actions.initBasePropertyOrder({
      linkId:detail.assetsId,
      orderType: detail.assetsType,
    }, res=>{
      const {owners, templateList, accountList, house, startTime, shop, parkingSpace} = res
      
      let myhouse = house
      if(detail.assetsType=="shops"){
        myhouse=shop
      }
      if(detail.assetsType=="parkingSpace"){
  ...
  handlenSubmit(){
    ...
    this.props.form.validateFieldsAndScroll((err, values)=>{
      if(!err){
        const {houseType, params, detail} = this.props
        const {fixType,director, linkPhone, linkName, fixStartTime } = values
        
        this.props.actions.EditFixPlan({
          id: detail.id,
          AssetsType: detail.assetsType,
          assetsId: detail.assetsId,
          ownerId: detail.ownerId,
          shopsTypeId: detail.shopsTypeId,
  ...
  render(){
    ...
    const {spinning, utils, visible, onCancel, showName, detail} = this.props
    const {owners, house} = this.state
    console.log(detail, "asa")
            ...
            <Col span={6}>
              <Form.Item  label="装修开始时间">
                {getFieldDecorator("fixStartTime", {
                  initialValue: detail.fixStartTime?moment(detail.fixStartTime):null
                })(
                  <DatePicker/>
                )}
              </Form.Item>
...
function mapDispatchProps(dispatch){
  return {
    actions: bindActionCreators({initBasePropertyOrder, initLoadOrderMoney,propertyPlanPage, EditFixPlan}, dispatch)
  }
}
...
export default connect(mapStateProps, mapDispatchProps)( Form.create()(EditTrim) )