...
import "./index.less"
import {getPlates, editCarAttaList, getCarAttaList} from "@/actions/otherAction"
...
class EditCar extends React.Component {
  ...
  componentDidMount(){
    this.props.actions.getCarAttaList({id: this.props.match.params.id}, res=>{
  ...
  handlenData(res){
    if(res.carCarparksList && res.carCarparksList.length){
      let carparkArr = []
      _.each(res.carCarparksList, item=>{
        carparkArr.push({
          carparkId: item.carparkId,
          time: item.validStartTime?[moment(item.validStartTime), moment(item.validEndTime)]:null
        })
      })
      this.setState({relaPlates: carparkArr})
    }
  }
  addPlate(){
  ...
  handlenSubmit(e){
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      if (!err) {
        ...
        this.props.actions.editCarAttaList({
          ...values,
  ...
  render(){
    ...
    const {relaPlates, plates, fileList, detail} = this.state
          ...
          <Form {...formItemLayout}>
            <Form.Item label="车牌号">
              {getFieldDecorator('licensePlate', {
                initialValue: detail.licensePlate,
                rules: [
                  {
                    required: true,
                    message: '车牌号!',
            ...
            <Form.Item label="业主">
              {getFieldDecorator('ownerId', {
                initialValue: detail.ownerName?{ownersName: detail.ownerName, ownersPhone: detail.ownerPhone,
                  ownersId: detail.ownerId}:"",
              })(<SearchHouse onlyHe />)}
            </Form.Item>
            <Form.Item label="联系人">
...
function mapDispatchProps(dispatch){
  return {
    actions: bindActionCreators({getPlates, editCarAttaList, getCarAttaList}, dispatch)
  }
}
...
export default connect(mapStateProps, mapDispatchProps)( Form.create()(EditCar) )