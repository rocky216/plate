...
import {editCompanyDevice, getCompanyDevice} from "@/actions/companyAction"
...
class EditCompany extends React.Component {
  ...
  componentDidMount(){
    ...
    this.props.actions.getDeviceDictTree({}, res=>{
      ...
      if(detail.deviceType && detail.deviceBrand){
        this.handlenChange([detail.deviceType, detail.deviceBrand])
      }
  ...
  handlenSubmit(){
    this.props.form.validateFieldsAndScroll((err, values)=>{
      if(!err){
        ...
        this.props.actions.editCompanyDevice({
            ...
            <Col span={12}>
              <Form.Item label="设备序列号" hasFeedback>
                {getFieldDecorator('deviceSerial', {
                  initialValue: detail.deviceSerial,
                  rules: [
                    {
                      required: true,
                      message: '填写设备序列号!',
            ...
            <Col span={12}>
              <Form.Item label="设备KEY" hasFeedback>
                {getFieldDecorator('deviceKey', {
                  initialValue: detail.deviceKey,
                })(<Input />)}
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item label="设备Secret" hasFeedback>
                {getFieldDecorator('deviceSecret', {
                  initialValue: detail.deviceSecret,
                })(<Input />)}
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item label="设备类型/品牌" hasFeedback>
                {getFieldDecorator('deviceType', {
                  initialValue: detail.deviceType && detail.deviceBrand?[detail.deviceType,detail.deviceBrand]:[],
                })(
              ...
              <Col span={12} key={item.value}>
                <Form.Item label={item.label} hasFeedback>
                  {getFieldDecorator(item.value, {
                    initialValue: detail?detail[item.value]:"",
                    rules: [
                      {
...
function mapDispatchProps(dispatch){
  return {
    actions: bindActionCreators({editCompanyDevice, getCompanyDevice, getDeviceDictTree}, dispatch)
  }
}
...
export default withRouter(connect(mapStateProps, mapDispatchProps)(Form.create()(EditCompany)))